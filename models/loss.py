...
class NoiseAwareLoss(nn.Module):
    ...
    def forward(self, y_pred, ycrf, yret, feature_map, classifier_weight):
        ...
        loss_wce = self.get_loss_wce(y_pred, feature_map, classifier_weight)
        return loss_ce + self.lambda_wgt * loss_wce
    ...
    def get_loss_wce(self, y_pred, feature_map, classifier_weight):

        # Correlation Map
        correlation_map = torch.zeros((self.num_classes+1, feature_map.shape[1], feature_map.shape[2]))
        for i in range(self.num_classes+1):
          correlation_map[i] = 1 + self.cos(feature_map, classifier_weight[i])

        # Confidence Map
        correlation_map_cstar = torch.zeros((feature_map.shape[1], feature_map.shape[2]))
        for i in range(self.num_classes+1):
          idx = ( ycrf == i )
